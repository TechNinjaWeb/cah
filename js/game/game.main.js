...
var GAME = function() {
	return (function(g,ch){
		var player = ch.player,
			czar = ch.czar;
		var state = g.state = {}, updateLoop = g.updateLoop = [], whiteCards = g.whiteCards = [], blackCards = g.blackCards = [], defaults, 
		...
		function dealCards(num, cards) {
			...
			game.players.forEach(function(e,i,a){
				...
				var tenCardHand = game.drawManyCards(cards, num);
		...
		function chooseCardCzar(p, options) {
			...
			Array.prototype.forEach.call(g.blackCards[0], function(e,i,a){
				...
				g.czar.cards.push(drawManyCards(game.blackCards, 1));
			});
